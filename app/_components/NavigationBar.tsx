...
export default function NavigationBar() {
  ...
  const isAdmin = userId === getEnv('VITE_ADMIN_USER_ID', 'NEXT_PUBLIC_ADMIN_USER_ID')
  ...
  return (
    <nav className="bg-primary text-primary-foreground sticky top-0 z-50 shadow-md">
      <div className="max-w-screen-xl flex items-stretch justify-between mx-auto px-4">
        <Link href="/" className="flex space-x-1 py-4">
      ...
      {menuOpen && (
        <div className="md:hidden bg-primary border-t border-primary-foreground/20">
          <ul className="flex flex-col px-4 py-2">
            <li>
              <button onClick={() => handleNav('/')} className="block w-full text-left py-3 px-2 text-primary-foreground hover:opacity-80">
                Hjem
              </button>
            </li>
            <li>
              <button onClick={() => handleNav('/oppskrifter')} className="block w-full text-left py-3 px-2 text-primary-foreground hover:opacity-80">
                Oppskrifter
              </button>
            </li>
            {isAdmin && (
              <li>
                <button onClick={() => handleNav('/admin/')} className="block w-full text-left py-3 px-2 text-primary-foreground hover:opacity-80">
                  Admin Panel
                </button>
              </li>
            )}
            <li>
              {session ? (
                <button onClick={handleSignOut} className="block w-full text-left py-3 px-2 text-primary-foreground hover:opacity-80">
                  Logg ut
                </button>
              ) : (
                <button onClick={() => handleNav('/signin')} className="block w-full text-left py-3 px-2 text-primary-foreground hover:opacity-80">
                  Logg inn
                </button>
              )}
            </li>
